...
def load_repo_documents(source_dir: Path):
    # Load documents
    # Python
    ...
    py_loader = GenericLoader(
        blob_loader=FileSystemBlobLoader(
            str(source_dir), glob="**/*.py", suffixes=[".py"], show_progress=True
        ),
    ...
    for glob_pattern in text_globs:
        try:
            loader = GenericLoader(
                ...
                blob_parser=LanguageParser(language=None)
            )
            text_docs.extend(loader.load())
        except Exception:
            logger.warning(f"No files found for pattern: {glob_pattern}")

    return py_loader.load() + text_docs


def process_chunks(documents, source_dir: Path, repo_name: str):
    # split documents, add metadata
    splitter = RecursiveCharacterTextSplitter.from_language(
        language=Language.PYTHON,
        chunk_size=1500,
        chunk_overlap=200
    )  # for better code context and catch indentation/headers
...
def save_hybrid_index(chunks, repo_name: str, save_path: Path):
    # save embeddings and BM25
...
def run_indexing(data_path: str = "data/fake_repo_agent"):
    # Entry point
    source_dir = Path(data_path).resolve()
    ...
    # Load
    all_docs = load_repo_documents(source_dir)
    if not all_docs:
        logger.warning("No indexable files found.")
        return None

    # Chunk
    chunks = process_chunks(all_docs, source_dir, repo_name)

    # Store
    try:
        return save_hybrid_index(chunks, repo_name, save_path)
    except Exception as e:
